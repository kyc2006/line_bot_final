...
`/test` 會回傳 `parsed_intent`、`extracted_entities`、`quick_reply_count` 與每則 reply 的 `quick_reply_texts`，可用來確認按鈕文字是否能被 dispatcher 正確解析。
...
- 停車場若沒有剩餘車位數，但仍有停車場名稱或地址，卡片會保留並顯示 `資料更新中` 與 `暫無即時車位`，讓使用者知道該停車場存在。
- 若有總車位，卡片會顯示 `總車位` 與使用率；若有地址、更新時間、收費或營業時間，才顯示對應 row。
- 若資料來源只提供燈號而沒有剩餘格數，Bot 不會把燈號假裝成剩餘數量；畫面會保留名稱、地址、總車位等實際有的資料，主資訊顯示資料更新中。
